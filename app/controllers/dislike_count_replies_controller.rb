class DislikeCountRepliesController < ApplicationController
  before_action :set_dislike_count_reply, only: [:show, :edit, :update, :destroy]
  ...
  def update
    respond_to do |format|
      if @dislike_count_reply.update(dislike_count_reply_params)
        format.html { redirect_to @dislike_count_reply, notice: 'Dislike count reply was successfully updated.' }
        format.json { render :show, status: :ok, location: @dislike_count_reply }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to dislike_count_replies_url, notice: 'Dislike count reply was successfully destroyed.' }
